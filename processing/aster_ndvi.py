# -*- coding: utf-8 -*-
"""ASTER NDVI calculation algorithm."""

import numpy as np
from osgeo import gdal
from qgis.core import (
    QgsProcessingAlgorithm,
    QgsProcessingParameterRasterLayer,
    QgsProcessingParameterRasterDestination,
    QgsProcessingException,
)
from ..core.raster_utils import create_output_raster, iterate_blocks, finalize_raster, count_blocks


class AsterNdviAlgorithm(QgsProcessingAlgorithm):
    VNIR_BAND = 'VNIR_BAND'
    OUTPUT = 'OUTPUT'

    def name(self):
        return 'aster_ndvi'

    def displayName(self):
        return 'ASTER NDVI'

    def group(self):
        return 'Vegetation Indices'

    def groupId(self):
        return 'vegetation_indices'

    def shortHelpString(self):
        return 'Calculates NDVI from an ASTER VNIR raster (band 2 = Red, band 3 = NIR).'

    def createInstance(self):
        return AsterNdviAlgorithm()

    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterRasterLayer(self.VNIR_BAND, 'ASTER VNIR raster'))
        self.addParameter(QgsProcessingParameterRasterDestination(self.OUTPUT, 'NDVI output'))

    def processAlgorithm(self, parameters, context, feedback):
        vnir_layer = self.parameterAsRasterLayer(parameters, self.VNIR_BAND, context)
        output_path = self.parameterAsOutputLayer(parameters, self.OUTPUT, context)

        ds_vnir = gdal.Open(vnir_layer.source(), gdal.GA_ReadOnly)
        if ds_vnir is None:
            raise QgsProcessingException('Failed to open VNIR raster')

        red_band = ds_vnir.GetRasterBand(2)
        nir_band = ds_vnir.GetRasterBand(3)
        cols = ds_vnir.RasterXSize
        rows = ds_vnir.RasterYSize

        out_ds = create_output_raster(output_path, ds_vnir)
        out_band = out_ds.GetRasterBand(1)

        total = count_blocks(rows, cols)
        current = 0

        for j, i, num_cols, num_rows in iterate_blocks(rows, cols):
            if feedback.isCanceled():
                break
            red_data = red_band.ReadAsArray(j, i, num_cols, num_rows).astype('f')
            nir_data = nir_band.ReadAsArray(j, i, num_cols, num_rows).astype('f')
            mask = np.greater(red_data + nir_data, 0)
            ndvi = np.choose(mask, (-99, (nir_data - red_data) / (nir_data + red_data)))
            out_band.WriteArray(ndvi, j, i)
            current += 1
            feedback.setProgress(int(current / total * 100))

        finalize_raster(out_ds)
        out_ds = None
        ds_vnir = None

        return {self.OUTPUT: output_path}
